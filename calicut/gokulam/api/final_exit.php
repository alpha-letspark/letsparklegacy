...
$sql = "select transaction_id,vehicle_type,entry_time,exit_time,amount,area from details_transaction where transaction_id = $transaction_id";
...
$updateSql= "UPDATE details_transaction SET status='1',amount='$amount',exit_time='$exit_time' , exit_user_id ='$exit_user_id' , exit_pay ='$amount_due'   where transaction_id = $transaction_id and vehicle_number = '$vehicle_number'";
...
$data = array("status" => success, "data" => $status , "calculatedfinalamount" => $amt,"final_entry_time"=> $entry_time,"final_exit_time"=> $exit_time,"vehicle_number" =>"$vehicle_number" , "gst" => "$gst" ,"amount_paid" => "$paid_amount" ,  "amount_due" => "$amount_due" , "area" => "$area" , "vehicle_number" => "$vehicle_number" );
}
