...
$sql = "SELECT count(*) as two_wheeler_count from details_transaction where exit_user_id = $employee_id  and DATE_FORMAT(exit_time,'%Y-%m-%d') >= '$date' and exit_time <= '$date1'   and  vehicle_type = '2' and area = '$area'";
$result3 = mysqli_query($con,$sql);
$two_wheeler_count1=mysqli_fetch_row($result3);
$two_wheeler_count = implode(',', $two_wheeler_count1); 


$sql1 = "SELECT count(*) as four_wheeler_count from details_transaction where exit_user_id = $employee_id  and DATE_FORMAT(exit_time,'%Y-%m-%d') >= '$date' and exit_time <= '$date1'  and  vehicle_type = '4' and area = '$area'";
...
$query1 = "SELECT COALESCE(SUM(exit_pay),0) as sum_exit_pay_two from details_transaction where exit_user_id = $employee_id   and DATE_FORMAT(exit_time,'%Y-%m-%d') >= '$date' and exit_time <= '$date1'   and vehicle_type = '2'  and area = '$area'";
$result1 = mysqli_query($con,$query1);
$sum_exit_pay_two=mysqli_fetch_row($result1);
$total2 = implode(',', $sum_exit_pay_two); 


$query2 = "SELECT COALESCE(SUM(exit_pay),0) as sum_exit_pay_four from details_transaction where exit_user_id = $employee_id   and DATE_FORMAT(exit_time,'%Y-%m-%d') >= '$date'  and exit_time <= '$date1'  and vehicle_type = '4'  and area = '$area'";
$result2 = mysqli_query($con,$query2);
$sum_exit_pay_four=mysqli_fetch_row($result2);
$total4 = implode(',', $sum_exit_pay_four);


$sql6 = "SELECT count(*) as foc_count from details_transaction where exit_user_id = $employee_id  and DATE_FORMAT(exit_time,'%Y-%m-%d') >= '$date' and exit_time <= '$date1' and  foc = '1' and  area = '$area'";
