...
function computeAmountFour($entryTime, $exitTime) {
  ...
  if(  $durationInSec <= 900 ) {
	...
	else if ( $durationInSec >= 7201  && $durationInSec <= 86400 ) 
...
<?php
function computeAmountPremium($entryTime, $exitTime) {
	...
	//If additional hour spent is there, need to add fare for that.
	if( $durationInSec > 0  && $durationInSec <= 7200 ) {
               $amount = 100;
 }

else if ( $durationInSec >= 7200  && $durationInSec <= 86400 ) 
	
	{
         $amount = 100 + ($addDuration * $rate) ;
	}
...
if(mysql_num_rows($results) >= '1')
{
	...
	{
		$amount = 0;
 $updateSql= "UPDATE details_transaction SET status='1', exit_time='$exit_time' , exit_user_id ='$exit_user_id' , exit_pay ='0', amount = '0'  where card_number = '$card_number' AND status= 0 ";
