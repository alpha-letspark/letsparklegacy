...
function computeAmountFourOvernight($entryTime, $exitTime) {
	//Calculating time spent in hour by vehicle.
	//$durationInHr = ceil((strtotime($exitTime) - strtotime($entryTime))/3600);
    //$durationInSec = ceil(strtotime($exitTime) - strtotime($entryTime));
...
function computeAmountTwoOvernight($entryTime, $exitTime) {
	//Calculating time spent in hour by vehicle.
	//$durationInHr = ceil((strtotime($exitTime) - strtotime($entryTime))/3600);
    //$durationInSec = ceil(strtotime($exitTime) - strtotime($entryTime));
    $date1 = strtotime($entryTime);  
...
{
	$amt = computeAmountFourOvernight($entry_time, $exit_time);
}
elseif (($card_type =="5") && $area == 0)
{
	$amt = computeAmountTwoOvernight($entry_time, $exit_time);
}
...
{
   $updateSql= "UPDATE details_transaction SET status='1',amount='$amount', exit_time='$exit_time' , exit_user_id ='$exit_user_id' , exit_pay ='$amount_due' , overnight_parking = '1'  where card_number = '$card_number' AND status= 0";
