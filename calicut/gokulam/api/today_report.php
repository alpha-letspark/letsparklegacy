...
$sql = "SELECT COUNT(redemption_id) as count FROM  redemption_shopowner where DATE_FORMAT(created_at,'%Y-%m-%d') = '$curdate' ";
$result3 = mysqli_query($con,$sql);
$total_redemmed_transaction=mysqli_fetch_row($result3);

$sql1 = "SELECT SUM(amount_redemmed) as count FROM  redemption_shopowner where DATE_FORMAT(created_at,'%Y-%m-%d') = '$curdate' ";
$result4 = mysqli_query($con,$sql1);
$total_amount_redemmed=mysqli_fetch_row($result4);

$query1 = "SELECT COUNT(redemption_id) as count FROM  redemption_shopowner where DATE_FORMAT(created_at,'%Y-%m-%d') = '$curdate' and vehicle_type = '2'";
$result1 = mysqli_query($con,$query1);
$two_wheeler_transaction_redemmed=mysqli_fetch_row($result1);

if($result1){

$query2 = "SELECT COUNT(redemption_id) as count FROM  redemption_shopowner where DATE_FORMAT(created_at,'%Y-%m-%d') = '$curdate' and vehicle_type = '4'";
$result2 = mysqli_query($con,$query2);
$four_wheeler_transaction_redemmed=mysqli_fetch_row($result2);

$query3 = "SELECT SUM(amount_redemmed) as count FROM  redemption_shopowner where DATE_FORMAT(created_at,'%Y-%m-%d') = '$curdate' and vehicle_type = '2'";
$result3 = mysqli_query($con,$query3);
$amount_from_two_wheeler=mysqli_fetch_row($result3);

$query4 = "SELECT SUM(amount_redemmed) as count FROM  redemption_shopowner where DATE_FORMAT(created_at,'%Y-%m-%d') = '$curdate' and vehicle_type = '4'";
