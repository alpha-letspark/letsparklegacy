...
$record = mysql_query( "SELECT * FROM details_transaction WHERE card_number = '$card_number' ");
   ...
   // if($seconds >= 1 || $q == 0 )
       if($record )
    {
      $query = "INSERT INTO details_transaction (userid,transaction_id,vehicle_type,entry_time,amount,client_id,vehicle_number,redemption_id,flag,status,area,card_number,card_type,entry_pay) 
SELECT * FROM (SELECT '$client_id' as userid,'$transaction_id' as transaction_id,'$vehicle_type' as vehicle_type,'$entry_time' as entry_time,'$amount' as amount,'$client_id' as client_id,'$vehicle_number' as vehicle_number,'$redemption_id' as redemption_id,'$flag' as flag,'$statusnew' as statusnew,'$area' as area , '$card_number' as card_number, '$card_type' as card_type,'$amount' as entry_pay) AS tmp
WHERE NOT EXISTS (
    SELECT entry_time FROM details_transaction WHERE entry_time >= '$entry_time1' and entry_time <= '$entry_time'  and card_number = '$card_number'  and status = '0'
);";
