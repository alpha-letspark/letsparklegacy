...
elseif ($vehicle_number != "" && $card_number != "")
{
	$sql = "SELECT transaction_id,vehicle_number,vehicle_type,entry_time,exit_time,amount , ROUND((amount/1.18),2) as price , ROUND((amount - amount/1.18),2) as gst,card_number FROM details_transaction where DATE_FORMAT(entry_time,'%Y-%m-%d') = '$curdate'  and  card_number = '$card_number' and vehicle_number = '$vehicle_number' and status = '0' ";
	$select = mysql_query($sql);
	$status = array();
	while($data = mysql_fetch_assoc($select)) {
				     $amount = $data['amount'];
