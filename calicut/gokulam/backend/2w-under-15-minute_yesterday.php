            ...
            <nav class="navbar navbar-transparent navbar-absolute">
                <div class="container-fluid">
                    ...
                    <div class="navbar-header">
                        ...
                        <a class="navbar-brand" href="#">2W Transaction under 15 min List view</a>
                    </div>
...
if($card_type == '7')
{			
$query = ("SELECT details_transaction.transaction_id,details_transaction.vehicle_number,details_transaction.card_number,details_transaction.redemption_id,details_transaction.vehicle_type,details_transaction.vehicle_number,
...
(TIME_TO_SEC(details_transaction.exit_time) - TIME_TO_SEC(details_transaction.entry_time)) AS duration, (CASE WHEN details_transaction.status = '1'
 ...
 DATE_FORMAT(details_transaction.entry_time,'%Y-%m-%d') = '$yesterday' and vehicle_type = '2' and (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) >= 0 and (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) <= 900 ");
}		
elseif($card_type == '6')
{
...
(TIME_TO_SEC(details_transaction.exit_time) - TIME_TO_SEC(details_transaction.entry_time)) AS duration, (CASE WHEN details_transaction.status = '1'
 ...
 (CASE WHEN card_details.type = '1' THEN 'Privilage'  WHEN card_details.type = '2' THEN 'Free'  WHEN card_details.type = '3' THEN 'Employee'  WHEN card_details.type = '4' THEN '4-Wheeler' WHEN card_details.type = '5' THEN '2-Wheeler' END ) as type 
 FROM details_transaction INNER JOIN card_details where details_transaction.card_number = card_details.card_number AND 
 DATE_FORMAT(details_transaction.entry_time,'%Y-%m-%d') = '$yesterday' and card_details.type = '$card_type' and vehicle_type = '2' and (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) >= 0 and (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) <= 900 ");
}
else
{			
$query = ("SELECT details_transaction.transaction_id,details_transaction.vehicle_number,details_transaction.card_number,details_transaction.redemption_id,details_transaction.vehicle_type,details_transaction.vehicle_number,
...
(TIME_TO_SEC(details_transaction.exit_time) - TIME_TO_SEC(details_transaction.entry_time)) AS duration, (CASE WHEN details_transaction.status = '1'
 ...
 (CASE WHEN card_details.type = '1' THEN 'Privilage'  WHEN card_details.type = '2' THEN 'Free'  WHEN card_details.type = '3' THEN 'Employee'  WHEN card_details.type = '4' THEN '4-Wheeler' WHEN card_details.type = '5' THEN '2-Wheeler' END ) as type 
 FROM details_transaction INNER JOIN card_details where details_transaction.card_number = card_details.card_number AND 
 DATE_FORMAT(details_transaction.entry_time,'%Y-%m-%d') = '$yesterday' and vehicle_type = '2' and (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) >= 0 and (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) <= 900 and (details_transaction.card_type = '4' or details_transaction.card_type = '5') ");
}			
	                                                                                                           //echo $query;
   
			
/*	$query = ("SELECT transaction_id,redemption_id,vehicle_type,vehicle_number,amount,entry_time, exit_time, (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) AS duration, (CASE WHEN status = '1' THEN 'Completed' ELSE 'Running' END ) as status,  (CASE WHEN area = '0' THEN 'Basement' ELSE 'Surface' END ) as area
               FROM details_transaction where DATE_FORMAT(entry_time,'%Y-%m-%d') = '$curdate' and vehicle_type = 2 and (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) >= 0 and (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) <= 900 ORDER BY ID DESC");
*/
