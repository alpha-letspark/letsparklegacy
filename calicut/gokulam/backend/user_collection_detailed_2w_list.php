...
// $user_id =  $_SESSION['id'];
  ...
  $query = ("SELECT transaction_id,redemption_id,vehicle_type,vehicle_number,entry_time,exit_pay, exit_time,cms_users.name as name,details_transaction.userid as entry_id,details_transaction.exit_user_id as exit_id, (TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) AS duration, (CASE WHEN details_transaction.status = '1' THEN 'Completed' ELSE 'Running' END ) as status 
               FROM details_transaction inner join cms_users where cms_users.id = details_transaction.exit_user_id and DATE_FORMAT(exit_time,'%Y-%m-%d') >= '$date1' and DATE_FORMAT(exit_time,'%Y-%m-%d') <= '$date2' and details_transaction.exit_user_id = '$employee_id' and vehicle_type = '2'");
