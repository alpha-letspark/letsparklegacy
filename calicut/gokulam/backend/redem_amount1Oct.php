...
{ 
 ...
 $curdate=  date('Y-m-d');
	 $result = mysql_query("SELECT * FROM details_transaction where (redemption_id LIKE '" . mysql_real_escape_string(trim($_GET['textfield2'])) . "'  and DATE_FORMAT(entry_time,'%Y-%m-%d') = '$curdate')") 
	 or die ("Sorry no results. Please try again.");
$redemption_id = $_GET['textfield2'];
echo $redemption_id;
	  $result1 = mysql_query("SELECT * FROM redemption_shopowner where redemption_id = '$redemption_id'  ")
	 or die ("Sorry no results. Please try again.");
...
<?php if(mysql_num_rows($result) >= 1 )
{
  //echo "<p><b>Search Result for : <b style='text-align:center;color:#FF9900;font-size:17px;'>$_GET[textfield2]</b></b><br />" ;
	...
	?>
      ...
      while($rowc = mysql_fetch_array($result) or  $rowc1 = mysql_fetch_array($result1) )
		  
	
  {
	  
  	$name = $rowc['vehicle_type'];
	$amount_redemmed = $rowc1['amount_redemmed'];
	echo $amount_redemmed;
echo $name;
...
if($name == "2"){
...
else if ($name == "4" || $amount_redemmed == "0" ) {
   $amount = array("10", "20");
    }
	
else if ($name == "4" || $amount_redemmed == "10" ) {
   $amount = array("10");
    }
else if ($name == "4" || $amount_redemmed == "10" ) {
   $amount = array("10");
 ...
 <form class="form-signin" action="redem_successfully.php" method="post" >

                                         <input type="hidden" name="redemption_id" class="form-control"  value="<?php echo $rowc["redemption_id"];?>" >
										 <input type="hidden" name="vehicle_type" class="form-control" value="<?php echo $rowc["vehicle_type"];?>" readonly>
...
?>
	 <input type="submit" style="position: relative;bottom: -19px;background-color: #ff7900;margin-left: 44%;top: -30px;"name="amount"value="<?php hello($name1); ?>"class="btn btn-secondary"></input> 
