...
$combined1 = date('Y-m-d H:i:s', strtotime("$date1 $time1"));
$combined2 = date('Y-m-d H:i:s', strtotime("$date2 $time2"));
...
<?php if($_SESSION["id"]==1 ){ ?>
 ...
 //$user_id =  $_SESSION['id'];
	...
	$query = sprintf("SELECT transaction_id,  vehicle_type, entry_time, exit_time, amount, "
			."(TIME_TO_SEC(exit_time) - TIME_TO_SEC(entry_time)) AS duration,"
			."CASE WHEN exit_time IS NOT NULL AND exit_time >= entry_time THEN 'Completed' ELSE 'Running' END AS status "
			."FROM details_transaction where  entry_time between '$combined1' AND '$combined2' ");
//	echo $query;
...
?> 
 ...
 <?php 
//	$user_id =  $_SESSION['id'];
	$query1 = sprintf("SELECT SUM(rating*actual_share*0.20) as total1 FROM  rating  where department='PMT' and topic='team' and user_id='9' and date = '$date' ");
	//echo $query1; 
		...
		//$total = $row['rating'] * $row['actual_share'];
		 ...
		 // echo $mark ;
		  ...
		  // echo "jaffar";
		  	...
		  	$query1 = sprintf("SELECT SUM(rating*actual_share*0.35) as total2 FROM  rating  where department='PMT' and topic='OPERATIONAL PLANNING' and user_id='9' and date = '$date'");
	//echo $query1; 
	$qu1 =  mysql_query($query1) or die(mysql_error());
	while($row1 = mysql_fetch_array($qu1)){
		//extract($rowc); 
		//$total = $row['rating'] * $row['actual_share'];
		 ...
		 // echo $mark ;
		  ...
		  // echo "jaffar";
		  	...
		  	$query1 = sprintf("SELECT SUM(rating*actual_share*0.25) as total3 FROM  rating  where department='PMT' and topic='FINANCIALS' and user_id='9' and date = '$date'");
	//echo $query1; 
	$qu1 =  mysql_query($query1) or die(mysql_error());
	while($row1 = mysql_fetch_array($qu1)){
		//extract($rowc); 
		//$total = $row['rating'] * $row['actual_share'];
		 ...
		 // echo $mark ;
		  ...
		  // echo "jaffar";
			...
			$query1 = sprintf("SELECT SUM(rating*actual_share*0.15) as total4 FROM  rating  where department='PMT' and topic='TECHNOLOGY USAGE' and user_id='9' and date = '$date' ");
	//echo $query1; 
	$qu1 =  mysql_query($query1) or die(mysql_error());
	while($row1 = mysql_fetch_array($qu1)){
		//extract($rowc); 
		//$total = $row['rating'] * $row['actual_share'];
		 ...
		 // echo $mark ;
		  ...
		  // echo "jaffar";  
		  	...
		  	$query1 = sprintf("SELECT SUM(rating*actual_share*0.05) as total5 FROM  rating  where department='PMT' and topic='BRAND IMAGE' and user_id='9' and date = '$date'");
	//echo $query1; 
	$qu1 =  mysql_query($query1) or die(mysql_error());
	while($row1 = mysql_fetch_array($qu1)){
		//extract($rowc); 
